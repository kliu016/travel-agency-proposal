#!/usr/bin/env python3
"""
Migrate vendor_websites table to match CSV structure
5 changes needed, 43 rows
"""
...
class VendorWebsitesMigration:
    def __init__(self):
        self.connection = None
        self.cursor = None
        self.csv_file = 'Vendor Websites-Grid view.csv'
        self.table_name = 'vendor_websites'
    ...
    def update_schema(self):
        ...
        try:
            ...
            renames = [
                ...
                ("vendor_type", "Vendor Type", "VARCHAR(100)")
            ]
    ...
    def migrate_data(self):
        ...
        for _, row in df.iterrows():
            record = {
                'Name': row['Name'],
                'Vendor Type': row['Vendor Type'] if pd.notna(row['Vendor Type']) else None,
                'Website': row['Website'] if pd.notna(row['Website']) else None,
                'Login': row['Login'] if pd.notna(row['Login']) else None,
    ...
    def verify_migration(self):
        ...
        # Show sample
        self.cursor.execute(f"SELECT `Name`, `Vendor Type`, `Website` FROM {self.table_name} LIMIT 3")
        results = self.cursor.fetchall()
        
        if results:
            logging.info("\nSample records:")
            for row in results:
                logging.info(f"  {row[0]} | Type: {row[1]} | Website: {row[2]}")

def main():
    migration = VendorWebsitesMigration()
    ...
    try:
        ...
        logging.info("\n✅ Vendor websites migration completed successfully!")
...
if __name__ == "__main__":
    logging.info("=== VENDOR WEBSITES MIGRATION ===")
    main()